...
ProcMap KittyMemory::getLibraryMap(const char *libraryName) {
    ...
    /*if ((strstr(libraryName, "libil2cpp.so") || strstr(libraryName, "libunity.so")) && fp != NULL) {
        while (fgets(line, sizeof(line), fp)) {
            if (strstr(line, OBFUSCATE("r--p")) && strstr(line, libraryName)) {
                ...
                if (strstr(libraryName, "libil2cpp.so")) {
                    if (libi = 1) {
                        break;
                    }
                    libi += 1;
                } else if (strstr(libraryName, "libunity.so")) {
                    if (libu = 1) {
                        break;
                    }
                    libu += 1;
        ...
        fclose(fp);
    } else if ((!strstr(libraryName, "libil2cpp.so") || !strstr(libraryName, "libunity.so")) && fp != NULL) {*/
        while (fgets(line, sizeof(line), fp)) {
            if (!strstr(line, OBFUSCATE("r--p")) && strstr(line, libraryName)) {
                char tmpPerms[5] = {0}, tmpDev[12] = {0}, tmpPathname[1024] = {0};
                // parse a line in maps file
